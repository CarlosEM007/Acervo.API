...
public class LibraryItemController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Insert([FromBody] CreateLibraryItemDto dto)
    {
        var entity = new LibraryItem(dto.LibraryId, dto.BookId);
        var result = await _service.Insert(entity);
        return result.Succeeded ? Ok() : BadRequest(result.Error);
    }

    [HttpPut]
    public async Task<IActionResult> Update([FromBody] UpdateLibraryItemDto dto)
    {
